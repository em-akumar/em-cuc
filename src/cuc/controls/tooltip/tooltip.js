class Tooltip {
  constructor(element, options) {
    ...
    this.options.container =this.link; //options.container || document.body;
    ...
    this.close = function (e) {
      ...
      this.styleTooltip = function (pos) {
        ...
        if (/top/.test(placement)) { //TOP
          this.tooltip.style.top = this.link.offsetTop - toolDim.h + 'px';
        } else if (/bottom/.test(placement)) { //BOTTOM
          this.tooltip.style.top = this.link.offsetTop + linkDim.h + 'px';
        } else if (/left/.test(placement)) { //LEFT
          this.link.style.position='relative';
          console.log(toolDim.w,toolDim.h ,linkDim.h);
          this.tooltip.style.left =  (-1)*toolDim.w + 'px';
          this.tooltip.style.top =  (-1)*toolDim.h + 'px';
          //this.tooltip.style.top = this.link.offsetTop - toolDim.h / 2 + linkDim.h / 2 + 'px';
          //this.tooltip.style.left = this.rect.left + scrollXOffset - linkDim.w  + 'px';

        } else if (/right/.test(placement)) { //RIGHT
          this.link.style.position='relative';
          console.log(toolDim.w,toolDim.h ,linkDim.h);
          this.tooltip.style.left =  linkDim.w + 'px';
          this.tooltip.style.top =  (-1)*toolDim.h + 'px';
          //this.tooltip.style.top = this.link.offsetTop - toolDim.h / 2 + linkDim.h / 2 + 'px';
          //this.tooltip.style.left = this.rect.left + scrollXOffset + linkDim.w + 'px';
        }
  ...
  static load() {
    ...
    [].forEach.call(Tooltips, function (item, index) {
      var options = {};
      options.animation = 'slideNfade';//item.getAttribute('data-animation');
      options.placement = 'top';//item.getAttribute('data-placement');
      options.duration = 200;//item.getAttribute('data-duration');
      options.delay = 150;//item.getAttribute('data-delay');
